            ...
            for j in range(0, 8):
                for k in range(0, 8):
                    ...
                    sitk.WriteImage(savImg,
                                    'D:\zpx\CVT3D\dataset\processed\LPET' + '/' + file + '_cut' + str(cut_cnt) + '.img')
                    cut_cnt += 1
        ...
        for i in range(0, 8):
            for j in range(0, 8):
                for k in range(0, 8):
                    ...
                    savImg = sitk.GetImageFromArray(image_cut.transpose(2, 1, 0))
                    sitk.WriteImage(savImg,
                                    'D:\zpx\CVT3D\dataset\processed\HPET' + '/' + file + '_cut' + str(cut_cnt) + '.img')
                    cut_cnt += 1
if __name__ == '__main__':
    Datamake('D:\datasets\Brain_Tumor\LPET','D:\datasets\Brain_Tumor\HPET')
